...
def Fkt_Export_Rund_ohneWanddicke(startwinkel, schrittweite, rohrdurchmesser):
	...
	for stufe in gom.app.project.stages:
		...
		for i in range(winkelaufloesung.shape[0]):
			if winkelaufloesung[i] <= ergebnis[stufe.name]['Biegewinkel']:
				fh.write(str(winkelaufloesung[i]) + ';' + str(ergebnis[stufe.name]['Nebenachse'][i]) + ';' + str(ergebnis[stufe.name]['Hauptachse'][i]) + ';' + str(ergebnis[stufe.name]['Unrundheit'][i])  + ';' + str(ergebnis[stufe.name]['Einfall'][i]) + ';' + str(ergebnis[stufe.name]['Kruemmung(Winkel)'][i]) + ';' + '\n')
