...
def Fkt_Export_Rund(startwinkel, schrittweite, rohrdurchmesser):
	...
	for stufe in gom.app.project.stages:
		...
		for i in range(winkelaufloesung.shape[0]):
			...
			ergebnis[stufe.name]['Unrundheit_ungerundet'][i] = ((ergebnis[stufe.name]['Nebenachse'][i] - ergebnis[stufe.name]['Hauptachse'][i]) / rohrdurchmesser)*100
			...
			#-> Einfall berechnen
			ergebnis[stufe.name]['Einfall'][i] = np.round(ergebnis[stufe.name]['Radius_R(winkel)'][i] + (rohrdurchmesser / 2) - ergebnis[stufe.name]['Radius_Ra(winkel)'][i], 2)
			#==============
